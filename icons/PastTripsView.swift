                    ...
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: UIScreen.main.bounds.height * 0.025) {
                            ...
                            VStack(spacing: UIScreen.main.bounds.height * 0.012) {
                                ...
                                ForEach(parsedTrips) { tripInfo in
                                    NavigationLink(
                                        destination: ItineraryView(location: tripInfo.location, days: tripInfo.days, userUID: userUID, selectedTripType: tripInfo.types, planController: PlanController())
                                    ) {
                                        ZStack {
                                            ...
                                            HStack {
                                                VStack(alignment: .leading) {
                                                    ...
                                                    Text("\(tripInfo.days) \(tripInfo.days == 1 ? "day" : "days")")
                                                        .font(.body)
                                                        .fontWeight(.semibold)
                                                        .foregroundColor(.white)
...
//    }
    ...
    func loadPastTrips() {
        ...
        // Relaxed filter: show any non-empty, non-duplicate trip
        self.pastTrips = trips.filter { trip in
            let isNonEmpty = !trip.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            let isUnique = seenTrips.updateValue(true, forKey: trip) == nil
            if !isNonEmpty { print("Filtered out empty trip string") }
            if !isUnique { print("Filtered out duplicate trip: \(trip)") }
            return isNonEmpty && isUnique
        }
        print("Trips to display: \(self.pastTrips)")
    }
...
// Helper struct to hold parsed trip info
struct TripDisplayInfo: Identifiable, Hashable {
    let id = UUID()
    let tripString: String
    let location: String
    let days: Int
    let types: String
...
extension PastTripsView {
    var parsedTrips: [TripDisplayInfo] {
        pastTrips.map { trip in
            let components = trip.components(separatedBy: " - ")
            let location = components.first?.replacingOccurrences(of: "Trip to ", with: "").trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            let cityName = location.split(separator: ",").first?.lowercased().replacingOccurrences(of: " ", with: "") ?? ""
            let dayAndType = components.count > 1 ? components[1].replacingOccurrences(of: " days", with: "").components(separatedBy: " (") : ["5", ""]
            let days = Int(dayAndType.first?.trimmingCharacters(in: .whitespacesAndNewlines) ?? "5") ?? 5
            let types = dayAndType.count > 1 ? dayAndType[1].replacingOccurrences(of: ")", with: "").trimmingCharacters(in: .whitespacesAndNewlines) : ""
            let daysType = (days % 2 == 0) ? "Even" : "Odd"
            let imageName = "\(cityName.prefix(1).lowercased())\(daysType)"
            return TripDisplayInfo(tripString: trip, location: location, days: days, types: types, imageName: imageName)
        }
    }
}
